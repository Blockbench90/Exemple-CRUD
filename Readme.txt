Данное руководство написано для тех, кому это сможет пригодиться, в частности для меня в будущем. Ностальгия и все такое...

Проект является простым примером, так скажем базой, для развертывания полноценного фулстек приложения CRUD(create, read, update, delete)
на основе технологий
MongoDB - База данных
NodeJs + Express - Бекенд
React - фронтенд

Описание основных действий, по типу установка библиотек или настройки, будут находится в этом файле. Остальное, очень детально в виде коментариев в самом коде.
Знаю, часть всего этого, после клона репозитория можно решить с помощью npm install, но все же, так будет более понятнее.





1. Первым делом, после создания папки проекта и ее названия, запускаем команду npm init и проходим путь "регистрации" проекта:
package name: (в скобках предложит название автоматически, исходя из названия папки проекта), можно просто согласиться, нажав Enter
version: (1.2.бла бла бла) соглашаемся, Enter
entry point: (index.js) предлагает главный файл, в моем случае это не дефолтный, ввожу app.js
test command: вводим если надо, в моем случае пропуск
git repository: я создавал в самом конце, если вы создали зарание, введите
keywords: ключевые слова, я натыкал разных, а вы на свое усмотрение
author: Обязательно указать себя, а то мало ли))) шутка, на свое усмотрение, если че, можно подправить опосля
но коль взялись писать себя в авторы, не забудьте после имени в <> кавычках указать email, так принято, что уж тут поделать
Далее Enter и еще раз, пропускаем соглашаясь со всем, и  идем в package.json смотреть на данные, если есть желание

2. Создаем проект. В корне будет находится бекенд, а дальше в будущей папке разместим фронтенд
устанавливаем базовые зависимости npm install express mongoose
для разработки с флагом -D, чтобы попало в dependencies
npm install -D nodemon concurrently
nodemon - для самостоятельного обновления проекта в браузере
concurrently - одновременно запускать и бекенд и фронэнд

3. Для удобства, устанавливаем фичу config, для удобства с разными константами
npm i config
после установки создаем папку config
а в ней файл default.json, куда и будем подключать константы
4. Создание базы данных в облачном сервисе MongoDB
После регистрации на cloud.mongodb.com, создаем проект, создаем бесплатный кластер, не забывая указать подключение к ней как к приложению (Connect Your Application)
если вы укажите свой IP, то не забудьте менять его после каждого нового подключения, если у вас динамический IP, как в большинстве подключений.
К примеру, я работаю с ноута, и дома у меня одно подключение, а на работе другое, я меняю их во вкладке NetworkAccess, хотя если нужно, можно указать
подключение для любого IP, я просто завтыкал это сделать, так что...
Все что нужно, для подключения к базе данных, это предложеный вам в конце URL, который нужно вставить в файл config, заменив там свое имя и пароль для подключения

4.bcryptjs
npm i bcryptjs для установки
Небольшая библиотека, позволяющая шифровать данные, в проекте нужна, чтобы не хранить пароли в сыром виде.
Чтобы не было как в пословице "Login: admin, Password: admin"

5. Валидатор на беке
npm i express-validator
Тут все на поверхности, данный модуль позволяет создавать валидацию, не прибегая к созданию ее в ручную
Подключили, задали условия и все.
Нужна она для тех случаев, если вдруг придет пустое поле, или неправильный email,
в таких случаях мы покажем на фронте милое сообщение или выдиление поля, чтобы было понятно что к чему, и чей прокол

6.Token
npm i jsonwebtoken
Установится библиотека для создания уникального токена

7. Подключаем фронтент в проект
npx create-react-app client
client - название папки, где будет сформировано реакт приложение, назвать можно как душе угодно, но как и "Hello World" ... сами понимаете
фронт будет полностью на реакте с некоторыми библиотечками, для отображения, чтобы не заморачиваться над CSS


8. Настройка скриптов для разработки
Для удобства, чтобы не насиловать кучу терминалов, можно прописать в скриптах всего несколько команд, которые будут делать всю работу
за нас, а мы отделаемся всего одним вводом команды для запуска приложение в режиме разработки

Вот так будет выглядеть первый блок в package.json, в корне проекта, не в папке client, а в корне самого проекта
{
  "name": "mern",
  "version": "1.0.0",
  "description": "Mern app",
  "main": "app.js",
  "scripts": {
    "start": "node app.js",
    "server": "nodemon app.js",
    "client": "npm run start --prefix client",                  - --prefix означает, что скрипт нужно запустить в папке client
    "dev": "concurrently \"npm run server\" \"npm run client\"" - вот и пригодился пакет concurrently, с его помощью мы обьеденяем экранированием
  },                                                            - запуск сразу нескольких команд
И теперь, для того чтобы запустить проект, нам нужно лишь ввсести в терминал команду
npm run dev     запуститься nodemon, react и будет перезагружать браузер по изминению в файлах

9. Чтобы не заморачиваться над CSS, там вы уже сами для себя решите как быть, я установлю библиотеку материалайз, и буду тырить от туда уже готовые
компоненты
установка должна быть на фронт, тоесть в папку client, и если вы в терминале находитесь в корневой директории, то перейдите в client
npm install materialize-css@next
В дальнейшем, все имена стилей и некоторые дополненные стили, взяты чисто из сайта библиотеки
ссылка https://materializecss.com/

10. PROXY
Чтобы синхронизировать фронт и бек, поскольку они работают на разных портах, необходимо подключить переадресацию, тоесть прокси.
В client/package.json устаравливаем новое поле proxy
  "eslintConfig": {
    "extends": "react-app"
  },
  "proxy": "http://localhost:5000", - теперь наш фронт, будет слать запросы на этот порт, только в режиме продакшена, разработки
  "browserslist": {
    "production": [
      ">0.2%",
      "not dead",
      "not op_mini all"
    ],
    "development": [
      "last 1 chrome version",
      "last 1 firefox version",
      "last 1 safari version"
    ]
  }
}




























